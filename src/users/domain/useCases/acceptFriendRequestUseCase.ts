...
@injectable()
export class AcceptFriendRequestUseCase implements UseCase<SocialRequest, boolean> {
  constructor(
  ...
  call(request: SocialRequest): boolean {
    ...
    this.eventHandler.subscribe<User>(
      ...
      (payload: User) => {
        ...
        const title = this.translations.get(SocialEvents.friendRequestAccepted, payload.name);
        const body = this.translations.get(SocialEvents.friendRequestAccepted, newRequest.name);
        const notification = {
          id: v4(),
          userId: payload.id,
          title: title.value,
